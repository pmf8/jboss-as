...
public class InfinispanSubsystemParser_1_0 implements XMLElementReader<List<ModelNode>>, XMLElementWriter<SubsystemMarshallingContext> {
    ...
    private ModelNode parseContainer(XMLExtendedStreamReader reader, List<ModelNode> operations, ModelNode address) throws XMLStreamException {
        ...
        // set the container resource address
    ...
    public void writeContent(XMLExtendedStreamWriter writer, SubsystemMarshallingContext context) throws XMLStreamException {
        ...
        if (model.isDefined()) {
            // default-cache-container attribute
            ...
            for (Property entry: model.get(ModelKeys.CACHE_CONTAINER).asPropertyList()) {
                ...
                // write the alias child element
                if (container.hasDefined(ModelKeys.ALIAS)) {
                    writeAliasElement(writer, container);
                }

                // write the transport child element
                if (container.hasDefined(ModelKeys.TRANSPORT)) {
                    writeTransportElement(writer, container);
                }

                // write the local-cache child elements
                for (ModelNode cache: container.get(ModelKeys.LOCAL_CACHE).asList()) {
                    // sanity check
                    Configuration.CacheMode mode = Configuration.CacheMode.valueOf(cache.get(ModelKeys.MODE).asString());
                    if (mode.isClustered()) {
                        // throw an exception
                    }
                    // write the cache element
                    writer.writeStartElement(Element.LOCAL_CACHE.getLocalName());
                    writeCacheAttributes(writer, cache) ;
                    writeCacheChildElements(writer, cache) ;
                    writer.writeEndElement();
                }

                // write the invalidation-cache child elements
                for (ModelNode cache: container.get(ModelKeys.INVALIDATION_CACHE).asList()) {
                    // sanity check
                    Configuration.CacheMode mode = Configuration.CacheMode.valueOf(cache.get(ModelKeys.MODE).asString());
                ...
                // write the replicated-cache child elements
                for (ModelNode cache: container.get(ModelKeys.REPLICATED_CACHE).asList()) {
                    // sanity check
                    Configuration.CacheMode mode = Configuration.CacheMode.valueOf(cache.get(ModelKeys.MODE).asString());
                ...
                // write the distributed-cache child elements
                for (ModelNode cache: container.get(ModelKeys.DISTRIBUTED_CACHE).asList()) {
                    // sanity check
                    Configuration.CacheMode mode = Configuration.CacheMode.valueOf(cache.get(ModelKeys.MODE).asString());
                    if (!mode.isReplicated()) {
                        // throw an exception
                    ...
                    writeDistributedCacheAttributes(writer, cache);
                    writeClusteredCacheAttributes(writer, cache);
                    writeCacheAttributes(writer, cache);
                    writeCacheChildElements(writer, cache) ;
                    // close cache element
                ...
                // close cache-container element
                writer.writeEndElement();
            }
        }
        // close subsystem element
        writer.writeEndElement();
    }

    private void writeTransportElement(final XMLExtendedStreamWriter writer, final ModelNode container) throws XMLStreamException {
    ...
    private void writeAliasElement(final XMLExtendedStreamWriter writer, final ModelNode container) throws XMLStreamException {
    ...
    private void writeCacheAttributes(final XMLExtendedStreamWriter writer, final ModelNode cache) throws XMLStreamException {
    ...
    private void writeClusteredCacheAttributes(final XMLExtendedStreamWriter writer, final ModelNode cache) throws XMLStreamException {
        // get the cache mode
        Configuration.CacheMode mode = Configuration.CacheMode.valueOf(cache.get(ModelKeys.MODE).asString());
    ...
    private void writeDistributedCacheAttributes(final XMLExtendedStreamWriter writer, final ModelNode cache) throws XMLStreamException {
    ...
    private void writeCacheChildElements(final XMLExtendedStreamWriter writer, final ModelNode cache) throws XMLStreamException {

        // write the locking element
        ...
        // write the state transfer element
